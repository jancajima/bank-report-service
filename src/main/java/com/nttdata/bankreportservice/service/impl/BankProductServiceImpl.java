...
public class BankProductServiceImpl implements BankProductService {
    ...
    public Mono<CustomerDto> getCustomer(String customerId) {
        return this.webClient.build().get().uri("/customer/{id}", customerId)
    ...
    public Mono<BankProductSummaryDto> getBankProductByCustomer(String customerId) {
        Mono<CustomerDto> dataCustomer = getCustomer(customerId);
        ...
        return dataCustomer.flatMap( c-> {
            product.setCustomerName(c.getName());
            Mono.just(product);
